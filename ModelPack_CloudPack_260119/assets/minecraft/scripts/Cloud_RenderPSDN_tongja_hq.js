...
function init(par1, par2) {
      ...
      main = renderer.registerParts(new Parts("body", "bodyLED"));
	  if (renderer.getModelName().indexOf("__gerade") != -1){
      main1 = renderer.registerParts(new Parts("gerade"));
    }
	if (renderer.getModelName().indexOf("__even") != -1){
      main1 = renderer.registerParts(new Parts("even"));
    }
	if (renderer.getModelName().indexOf("__uper") != -1){
      upper = renderer.registerParts(new Parts("upper"));
    }
      door = renderer.registerParts(new Parts("doorL"));
	  doorR = renderer.registerParts(new Parts("doorR"));
	   colorL = renderer.registerParts(new Parts("colorL"));
	  colorR = renderer.registerParts(new Parts("colorR"));
      loff = renderer.registerParts(new Parts("L1"));
      lon = renderer.registerParts(new Parts("L2"));
	  if (renderer.getModelName().indexOf("__HDG_R") != -1){
      body1 = renderer.registerParts(new Parts("headingR"));
    }
		  if (renderer.getModelName().indexOf("__HDG_L") != -1){
      body1 = renderer.registerParts(new Parts("headingL"));
    }
	 if (renderer.getModelName().indexOf("__gerade") != -1){
     colorline = renderer.registerParts(new Parts("color", "color_gerade"));
    }
	if (renderer.getModelName().indexOf("__even") != -1){
      colorline = renderer.registerParts(new Parts("color", "color_even"));
    }
	...
	Dochak = renderer.registerParts(new Parts("Dochak"));	
...
function render(entity, pass, par3, color){
   ...
   if (renderer.getModelName().indexOf("RAIL") != -1){
	...
	main1.render(renderer); 
	body1.render(renderer);
	if (renderer.getModelName().indexOf("__uper") != -1){
      upper.render(renderer);
    }
   ...
   //수치 알아서 조절하십시오.
    ...
    var doorMoveDistanceA = 1.05; //ドアの長さ(m)
    var doorMoveTime = 3.2; //開閉するまでの時間
...
var world = entity.func_145831_w();
    ...
    var doorRender = function (door, Minus, MoveDis) {
      ...
      {  
        NGTRenderHelper.setColor(color);
        colorL.render(renderer);
        NGTRenderHelper.setColor(0xFFFFFF);
    }
    GL11.glPopMatrix();
    }
	
	    var doorRRender = function (door, Minus, MoveDis) {
        GL11.glPushMatrix();
        GL11.glTranslatef(doorMove * Minus * MoveDis, 0, 0);
     	 	doorR.render(renderer);
   ...
   //문이 열리네요~~~
        ...
        doorRRender(door, -flag, doorMoveDistanceA);
}
