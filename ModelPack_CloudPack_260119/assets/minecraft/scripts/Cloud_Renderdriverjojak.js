...
function init(par1, par2) {
      ...
      open_off = renderer.registerParts(new Parts('open_off'));
      open_on = renderer.registerParts(new Parts('open_on'));
	    close_off = renderer.registerParts(new Parts('close_off'));
      close_on = renderer.registerParts(new Parts('close_on'));
...
function render(entity, pass, par3){
   ...
   //수치 알아서 조절하십시오.
    ...
    var doorMoveDistanceA = 0.1; //ドアの長さ(m)
...
var world = entity.func_145831_w();
    ...
    doorMoveData[entityId] = doorMove;
	   ...
	   if (doorMove <= 0) {
		       GL11.glPushMatrix();
        close_on.render(renderer);
		open_off.render(renderer);
		GL11.glPopMatrix();
    }
	
    else if (0 < doorMove && doorMove < doorMoveDistance) {
		  
        if ( 0.5 < doorMove && doorMove <= 0.54) {
				GLHelper.disableLighting();
				 GL11.glPushMatrix();
            open_on.render(renderer);
			close_on.render(renderer);
			GL11.glPopMatrix();
